...
## 后续记录（2026-08-13，Phase 1 之后）

### UI 修复三处

| 位置 | 问题 | 归属 |
|---|---|---|
| Cookie 页快手行 | 内层 Row 缺 `expand`，输入框失去边框、文本溢出，扫码按钮被挤成左上角小方块 | 本轮引入的回归 |
| 账号设置页 | 9 处 `width=500` 固定宽超出内容区（950 窗口下可用约 493px）被右侧截断 | 上游既有 |
| 说明页功能亮点 | `spacing=100` 使 5 项总宽约 770px 超出卡片内容区约 634px，末两项溢出 | 上游既有 |
| 说明页简介文本 | `text_align=JUSTIFY` 在中文下只能于标点处拉伸，撑出行内空洞 | 上游既有 |

验证方式：Flet web 模式（950 视口，`is_mobile_device` 判为桌面，走同一布局分支）
配合 playwright 逐 tab 截图核对。此前两次「改完即宣告修好」都翻车，此后一律以截图为准。

### 用户配置被覆盖事故

**现象**：部署后 `Loaded 0 items`，轮询间隔回落默认 600s。

**链条**：为做 UI 验证从源码跑了 `main.py --web` → 源码模式下 `user_data_dir`
就是仓库根目录，程序在 `config/` 生成 5 个空的用户数据文件（均已 gitignore，
`git status` 干净因而未被察觉）→ 打包时 spec 整目录打包 `config/` 把空文件带进包
→ 启动时 `prepare_user_data_dir()` 的 `copytree(dirs_exist_ok=True)`
覆盖了 `%APPDATA%\StreamCap\config` 的真实数据。

**恢复**：C 盘卷影副本（2026-08-12 09:09）取回 6 个房间、1490 字符快手登录态、
450s/并发1/间隔3 设置；第 7 个房间晚于快照，按日志证据重建补回。

**加固**（三层）：
1. `paths.py` 的 copytree 增加 ignore 回调，5 个用户数据文件目标已存在则跳过
2. `StreamCap.spec` 改为逐个指定 3 个配置文件
3. `scripts/build.py` 同步改为逐个指定（上游脚本同样有此缺陷，
   影响所有从源码跑过一次的人）
...
| 来源 | 结论 |
|---|---|
| StreamCap #17（作者 ihmily） | 「根源上并未解决，只能在程序上减少对快手的请求」，建议用定时监控 |
| StreamCap #281 | 「IP banned 如何解决」，两人追问至今无回复 |
| DouyinLiveRecorder #862 | 实测 6000 秒轮询间隔照样被封 → 触发条件不是频率 |
| DouyinLiveRecorder #1058 | 提出「封的是 UA 不是 IP」 → **本地实测证伪**（见下） |

**本地实测（同一 IP、同一房间）**：
- 手机分享接口：换 5 个 iOS/Android/浏览器 UA 全部返回「操作太快了」，
  匿名请求同样被限流 → 限流维度既非 UA 也非账号
- 网页通道：1.5 秒间隔连打 12 次未触发限流 → 另有独立且宽松的配额

**落地改进**：手机接口冷却由固定 600s 改为指数退避（600s 起翻倍，封顶 4 小时，
取到真实数据即重置）。配额耗尽后固定 10 分钟重试等于每天白打上百次无效请求。

**已排除的方向**：UA 轮换（已证伪）、拉长 `loop_time`（社区实测无效）。
