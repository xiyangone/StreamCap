...
当前只维护 Rust 原生版。旧 Python/Flet、Docker Web 和旧 macOS 打包入口已退役。浏览器刷新不再触发直播检测，以免因反复刷新造成平台限流；请使用监控间隔或明确的手动检测。
...
TS 录中预览从最近约 4 MiB 开始，避免加载数小时录像；停止后可预览完整文件。兼容格式和直播源预览使用有界 FFmpeg 管道；关闭预览会取消并等待其进程退出，不改原文件。直播源缓存有效期 5 分钟，过期需手动检测，不会因预览或刷新自动增加平台请求。
