    ...
    view! {
        <article class="recording-card glass" class:selected=selected_now class:is-recording=move || record.get().is_recording data-rec-id=move || record.get().rec_id aria-label=move || record.get().name()>
            ...
            <div class="card-main">
                ...
                <p class="card-subtitle" title=move || record.get().live_title.unwrap_or_default()>{move || record.get().live_title.filter(|s| !s.is_empty()).unwrap_or_else(|| if record.get().monitor_status { t("监控中，开播后自动录制").into() } else { t("监控已暂停").into() })}</p>
            ...
            <div class="card-actions">
                <button class="button card-record" class:recording=move || record.get().is_recording disabled=move || busy.get() || !state.status.get().ok || (!record.get().is_recording && !state.status.get().resolver_ready) on:click=move |_| perform.run(Action::Record)>
                    <Show when=move || record.get().is_recording fallback=|| view! { <Icon name="play" size=14 /> }><Icon name="stop" size=14 /></Show>{move || if record.get().is_recording { t("停止录制") } else { t("开始录制") }}
                </button>
                <div class="card-tools">
                    <button class="icon-button" aria-label=t("检测直播状态") title=t("检测状态（已开播时会启动录制）") disabled=move || busy.get() || !state.status.get().resolver_ready on:click=move |_| perform.run(Action::Check)><Icon name="refresh" size=16 /></button>
                    <button class="icon-button" aria-label=t("预览录制文件") title=t("预览录制文件") on:click=move |_| on_preview.run(record.get_untracked())><Icon name="eye" size=16 /></button>
                    <button class="icon-button" aria-label=move || if record.get().monitor_status { t("暂停监控") } else { t("开启监控") } title=move || if record.get().monitor_status { t("暂停监控并停止录制") } else { t("开启监控") } disabled=move || busy.get() || !state.status.get().ok on:click=move |_| perform.run(Action::Monitor)>
