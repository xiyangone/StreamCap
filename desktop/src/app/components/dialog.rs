...
pub fn ConfirmDialog(
    ...
    busy: RwSignal<bool>,
    on_confirm: Callback<()>,
    #[prop(default = "确认删除")] confirm_text: &'static str,
    #[prop(default = "正在删除…")] busy_text: &'static str,
    #[prop(default = "此操作无法撤销，请确认选择范围。")] hint: &'static str,
) -> impl IntoView {
    view! {
        <Dialog open=Signal::derive(move || open.get()) title=title busy=busy on_close=Callback::new(move |_| open.set(false))>
            ...
            <footer class="modal-actions"><button class="button secondary" disabled=move || busy.get() on:click=move |_| open.set(false)>"取消"</button>
                <button class="button danger" disabled=move || busy.get() on:click=move |_| on_confirm.run(())>{move || if busy.get() { busy_text } else { confirm_text }}</button>
            </footer>
        </Dialog>
    }
}
