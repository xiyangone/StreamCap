...
pub fn AboutView() -> impl IntoView {
    ...
    view! {
        <div class="page about-page"><header class="page-header"><div><span class="eyebrow">"BUILT FOR THE MOMENTS"</span><h1>"关于 StreamCap"</h1><p>"记录直播，珍藏热爱。"</p></div></header>
            <section class="about-hero glass"><div class="about-brand-mark"><img src="/logo-mark.png" alt="StreamCap 熊猫标识" /></div><span class="eyebrow">"YOUR LIVE RECORDING WORKSPACE"</span><h2>"StreamCap"</h2><p>"多平台直播录制，本地保存，自在掌控。"</p><span class="version-badge">"桌面版 "{env!("CARGO_PKG_VERSION")}</span></section>
            <div class="about-grid"><section class="about-detail glass"><h3><Icon name="monitor" size=19 />"应用信息"</h3><dl class="info-list"><div><dt>"运行平台"</dt><dd>"Windows 11"</dd></div><div><dt>"本地服务版本"</dt><dd>{move || if state.status.get().version.is_empty() { "未连接".into() } else { state.status.get().version }}</dd></div><div><dt>"平台解析服务"</dt><dd>{move || if state.status.get().resolver_ready { "已就绪" } else { "未就绪" }}</dd></div><div><dt>"许可协议"</dt><dd>"Apache License 2.0"</dd></div></dl></section>
